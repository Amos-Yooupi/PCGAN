...
class TimeBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3):
        ...
        self.conv2 = nn.Conv2d(in_channels, out_channels, (1, kernel_size), padding=(0, kernel_size // 2))
        self.conv3 = nn.Conv2d(in_channels, out_channels, (1, kernel_size), padding=(0, kernel_size // 2))
...
class STGCN(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.block1 = STGCNBlock(in_channels=config.hidden_dim, out_channels=64,
                                 spatial_channels=16)
        self.block2 = STGCNBlock(in_channels=64, out_channels=64,
                                 spatial_channels=16)
        self.last_temporal = TimeBlock(in_channels=64, out_channels=64)
        self.fully = nn.Linear(64, config.obs_dim)
    ...
    def forward(self, train_data, bridge_data, earth_data, adj, *args):
        ...
        out3 = self.length_linear(out3.transpose(2, 3)).transpose(2, 3)
        out4 = self.fully(out3[:, :3])
        return out4.permute(0, 2, 1, 3)
